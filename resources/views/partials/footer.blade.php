<div class="container-fluid bg-blue text-white">
  <footer class="py-5 d-flex justify-content-between container">
    <div class="d-flex">
      <div class="me-3">
        <h5>Support</h5>
        <ul class="nav flex-column">
          <li class="nav-item mb-1"><a href="#" class="nav-link p-0 text-white">Customer Care</a></li>
          <li class="nav-item mb-1"><a href="#" class="nav-link p-0 text-white">Terms and Condition</a></li>
          <li class="nav-item mb-1"><a href="#" class="nav-link p-0 text-white">Frequently Asked Question (FAQ)</a></li>
        </ul>
        <p class="mt-3" style="font-size: 14px">&copy; 2022 BarTrade Indonesia Company. All rights reserved.</p>
      </div>
  
      <div class="">
        <h5>Follow Us</h5>
        <div class="d-flex">
          <a href="#" class="me-3"><i class="bi bi-instagram" style="font-size: 40px"></i></a>
          <a href="#" class="ms-3"><i class="bi bi-facebook" style="font-size: 40px"></i></a>
        </div>
      </div>
    </div>

    <div class="me-5">
      <a href="/" class="d-flex align-items-center mb-3 link-dark text-decoration-none">
        <img src="/image/logo.png" alt="" style="width: 200px">
      </a>
    </div>
  </footer>
</div>